...
DAILY_FOCUS_PROMPTS = {
    "gratitude": "🌟 За что я благодарен(а) сегодня? Напиши коротко.",
    "success": "✅ Что сегодня удалось? Какой момент прошёл особенно хорошо?",
    "value": "🎯 Что было для меня важным? На что сегодня было приятно обратить внимание?",
    "rest": "💤 Что поможет улучшить состояние? Напиши коротко.",
}
